@props(['id', 'type' => 'botton', 'bg' => 'bg-white focus:bg-gray-300 dark:bg-dark dark:text-white', 'border' => 'border border-gray-200 dark:border-neutral-700'])
...
@elseif($type && $type == 'dropdown-image')
<div class="dropdown relative">
    <div {{ $attributes->merge(['class' => "w-auto h-10 shadow-md rounded-full dropdown-toggle flex items-center justify-center $bg $border", 'href' => '#']) }} id="{{ $id }}" data-bs-toggle="dropdown" aria-expanded="false" data-mdb-ripple="true" data-mdb-ripple-color="dark">
        {{ $slot }}
    </div>
