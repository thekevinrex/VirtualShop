    ...
    <div class="flex flex-col items-start p-5 w-full">
        ...
        <section class="w-full" aria-labelledby="perso-modules-heading" aria-describedby="perso-modules-details">
            ...
            <h1 id="perso-modules-heading" class="flex flex-row justify-between">
                ...
                <Link slideover href="{{ route('seller.products.perso.create', $product) }}" class="py-1 px-3 rounded-md bg-transparent text-primary"  data-mdb-ripple="true" data-mdb-ripple-color="dark">@lang('product.add_module')</Link>                
            </h1>

            <p id="perso-modules-details" class="text-base mb-5 max-w-3xl">
                @lang('product.perso_modules_help')
            </p>
        ...
        <div class="space-y-5 mt-5 flex flex-col w-full">
            ...
            <productModulesComponent v-slot="{ modules, isEditModule, editModule, deleteModule, orderModule, handleSubmit }" :modulesData="{{ json_encode($modulesData) }}" :modulesInfo="{{ json_encode($modulesInfo) }}">
            
                <x-splade-errors>
                    <div class="flex flex-col bg-red-700 rounded-md" v-if="Object.keys(errors.all).length > 0">
                        <div class="w-full py-1 px-3 my-1 mx-1" v-for="(error, key) in Object.values(errors.all).flat()">
                ...
                <x-splade-form action="{{ route('seller.products.perso.store', $product) }}" method="post">
                    <button type="button" @click.prevent="handleSubmit(form)" class="rounded-md border border-transparent disabled:bg-indigo-300 bg-indigo-600 py-2 px-4 text-sm text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 font-bold focus:ring-indigo-500 focus:ring-offset-2" data-mdb-ripple="true" data-mdb-ripple-color="ligth">
                        @lang ('product.save')
                    </button>
                </x-splade-form>
                
                <div class="flex flex-col w-full border-b border-gray-300 dark:border-neutral-700" v-for="module in modules" :key="module.key">
                
                    <div class="w-full flex flex-row justify-between">

                        <div class="flex flex-row shrink-0 items-center space-x-2">
                            
                            <div v-if="module.order > 1"  @click="orderModule('up', module.key)" data-mdb-ripple="true" data-mdb-ripple-color="dark" class="w-10 h-10 rounded-full overflow-hidden flex items-center justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
                            ...
                            <div v-if="module.order < modules.length" @click="orderModule('down', module.key)" data-mdb-ripple="true" data-mdb-ripple-color="dark" class="w-10 h-10 rounded-full overflow-hidden flex items-center justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
                        ...
                        <div class="w-full flex flex-row justify-end">
                            ...
                            <div v-if="module.key != isEditModule" data-mdb-ripple="true" data-mdb-ripple-color="dark" @click="editModule(module.key)" class="w-10 h-10 rounded-full overflow-hidden flex items-center justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
                                    <path d="M21.731 2.269a2.625 2.625 0 00-3.712 0l-1.157 1.157 3.712 3.712 1.157-1.157a2.625 2.625 0 000-3.712zM19.513 8.199l-3.712-3.712-12.15 12.15a5.25 5.25 0 00-1.32 2.214l-.8 2.685a.75.75 0 00.933.933l2.685-.8a5.25 5.25 0 002.214-1.32L19.513 8.2z" />
                                </svg>                                                              
                            </div>

                            <div data-mdb-ripple="true" data-mdb-ripple-color="dark" @click="deleteModule(module.key)" class="w-10 h-10 rounded-full overflow-hidden flex items-center justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
                    ...
                    <div class="w-full flex flex-row flex-wrap py-4">
                        ...
                        <div class="w-1/2 max-w-[450px]">
                            <ModuleContent :module="module.type"></ModuleContent>
                        </div>

                        <div class="flex w-1/2 flex-col px-4 space-y-3" v-if="module.key != isEditModule">
                            
                            <section v-if="module.title != null">
                                <h2 class="text-lg font-semibold">
                                    @lang('product.title')
                                </h2>
                                <p class="text-base" v-text="module.title"></p>
                            </section>

                            <section v-if="module.des != null">
                                <h2 class="text-lg font-semibold">
                                    @lang('product.des')
                                </h2>
                                <p class="text-sm text-ellipsis" v-text="module.des"></p>
                            </section>

                            <section v-if="module.image != null">
                                <div class="w-24 h-24 rounded-md border overflow-hidden flex justify-center items-center relative">
                                    <img :src="module.image.path" class="w-full h-full" />
                                </div>
                            </section>
                        </div>

                        <div class="flex w-1/2 flex-col px-4 space-y-5" v-if="module.key == isEditModule">
                            ...
                            <section v-if="module.des != null">
                                <InputComponent v-model:value="module.des" type="textarea" :inputData="{
                                    'name' : 'des',
                                    'isHeader' : true,
                                    'placeholder' : '{{ trans('product.des') }}',
                                }" >
                                </InputComponent>
                            </section>

                            <section v-if="module.image != null">
                                <UploadComponent v-model:value="module.image" :multiple="false" accept="image/*" name="module-image" id="module-image">
                                    <template #default="{image, data}">
                                        <div class="w-24 h-24 rounded-md border overflow-hidden flex justify-center items-center relative" :class="(data.error)? 'border-red-600' : 'border-gray-300'">
                                            <label for="module-image" class="w-full h-full z-10 absolute top-0 left-0 right-0 bottom-0 flex items-center justify-center">
