...
<div class="sticky-top top-14 bg-white border-b border-gray-300 dark:bg-dark dark:border-neutral-700 dark:text-white flex w-full h-24 flex-col">
    ...
    <div class="flex flex-row flex-nowrap justify-between w-full h-12 items-center px-5">
        <div class="flex flex-row h-12 space-x-2 dark:text-white items-end">
            ...
            <div class="px-5 h-10 space-x-1 flex items-center rounded-t-md relative" data-mdb-ripple="true" data-mdb-ripple-color="dark">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
        ...
        <div class="flex flex-none justify-end px-5">
            <Link href="{{ route('seller.products.edit', $product) }}" class="py-1 px-3 rounded-md bg-transparent text-primary"  data-mdb-ripple="true" data-mdb-ripple-color="dark">
                {{__('Edit Product')}}
            </Link>
        </div>
...
@section('content')
    @includeWhen($page == null, 'seller.products.inventory.content')
@endsection