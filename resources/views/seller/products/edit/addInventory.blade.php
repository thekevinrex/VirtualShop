    ...
    <x-form class="space-y-6 bg-white dark:bg-dark" default="{ error : [] }" action="{{ route('seller.products.inventory.store', $product) }}" method="post">
            ...
            <div>
                @foreach ($states as $item )
                    <div class="py-1 px-2 space-x-2 flex items-center">
                        <input value="{{ $item['id'] }}" type="radio" name="state" id="state-{{ $item['id'] }}" v-model="form.state">
        ...
        <section aria-describedby="product-cant-details" aria-labelledby="product-cant-heading">
            ...
            <h2 id="product-cant-heading" class="text-lg font-semibold">
                @lang('product.inventory_cant')
            </h2>

            <p class="mb-2" id="product-cant-details">
                @lang('product.inventory_cant_help')
            </p>
            ...
            <div class="flex max-w-[150px] w-full">
                <InputComponent v-model:value="form.cant" type="text" :inputData="{
                    'name' : 'cant',
                    'isHeader' : false,
                    'placeholder' : '{{ trans('product.inventory_cant') }}',
                    'validate' : ['Required'],
                }" >
                </InputComponent>
            </div>
