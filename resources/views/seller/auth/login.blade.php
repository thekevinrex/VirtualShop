@extends('seller.layauts.auth-container')

@section('auth-form')

<h1 class="font-bold text-3xl dark:text-white">
    {{__('Sign in to seller panel')}}
</h1>

<x-form class="mt-4 space-y-6 bg-white dark:bg-dark" default="{ error : [] }" action="{{ url('/seller-panel/login') }}" method="post">
    ...
    <div class="flex flex-col">
        ...
        <InputComponent v-model:validation="form.error" v-model:value="form.username" type="text" :inputData="{
            'name' : 'username',
            'front_icon' : true,
            'isHeader' : true,
