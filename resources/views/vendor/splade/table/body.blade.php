<tbody class="divide-y divide-gray-200 bg-white dark:bg-dark">
    @forelse($table->resource as $itemKey => $item)
        ...
        <tr
            :class="{
                'bg-gray-50 dark:bg-neutral-700': table.striped && @js($itemKey) % 2,
                'hover:bg-gray-100 dark:hover:bg-neutral-800': table.striped,
                'hover:bg-gray-50 dark:hover:bg-neutral-500': !table.striped
            }"
        >
        ...
        <tr>
            <td colspan="{{ $table->columns()->count() }}" class="whitespace-nowrap">
                @if(isset($emptyState) && !!$emptyState)
                    {{ $emptyState }}
                @else
                    <p class="text-black dark:text-white px-6 py-12 font-medium text-sm text-center">
                        {{ trans('splade.empty') }}
                    </p>
                @endif
            </td>
